...
const initSocket = (server) => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("new_order", async (payload) => {
            try {
                ...
                try {
                    await session.withTransaction(async () => {
                        const orderDocs = Object.keys(grouped).map((shopId) => {
                            ...
                            const total = shopItems.reduce((s, it) => s + it.subtoal, 0);
                            return {
                ...
                createdOrders.forEach((order) => {
                    // Emit to Shop Admin
                    const shopRoom = `shop_${String(order.items[0].shopId)}`;
                    io.to(shopRoom).emit("order_created", order);

                    // Emit Global
                    // io.emit("order_created_global", order);

                    // Emit to User
                    const uid = order.user?.userId ?? order.userId ?? (order.user && order.user._id) ?? null;
                    if (uid) io.to(`user_${Stging(uid)}`).emit("order_created_for_user", order);
                });
            } catch (error) {
                console.error("new_order error: ", error);
                socket.emit("order_error", { message: error?.message || "Failed to place order", success: false });
            }
        });
        ...
        const handleUpdateStatus = async (orderId, status, emitterSocket = socket) => {
            ...
            try {
                if (!orderId) {
                    return emitterSocket.emit("order_error", { message: "orderId required", success: false });
                }
                
                const order = await Order.findById(orderId);
                
                if (!order) {
                    return emitterSocket.emit("order_error", { message: "Order not found", success: false });
                }
        ...
        socket.on("cancel_order", async ({ orderId }) => {
            await handleUpdateStatus(orderId, "cancalled", socket);
        });
